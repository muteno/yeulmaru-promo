...
1. **SSOT = `docs/디자인기틀.md`** — UI 작업 전 필독. 기틀에 있는 토큰(`:root` 2블록)과
   정본 컴포넌트(`.btn` 8변형·`.u-*`·`.sw`·`.icon-btn`·`.toast`·`.chip`·`.modal`·`nb-*`·`.rv` 등 **24종**)만 사용.
   ※ 개수 표기는 4곳 동기(이 줄 ↔ 기틀 §2 헤딩 ↔ `docs/절대명령2_정본인덱스.md` ③ 칸 ↔ `design_digest.py`) — 표가 정본.
...
4. **게이트**: `python3 tools/check_design.py`가 위반을 검사한다(커밋 시 `.githooks/pre-commit`이 강제).
   ...
   **정본 컴포넌트(버튼·칩·빈상태 등)를 손댔으면** `npm run smoke:parity` — 「클래스가 같으면 픽셀도 같아야 한다」
   래칫이다. 자리마다 인라인으로 다시 튜닝하지 말고 정본 CSS를 고쳐라(값을 새로 정해야 하면 운영자에게 질문).
