...
### 📋 백로그 큐 (사용자 지시)
1. **연간일정을 promo 안에 통합** (현재 iframe = 별도 레포 `yeulmaru-calandar-2026`) — "일단 반영하고 나중에 통합" 지시.
2. 통합 전까지 캘린더 레포 수정 2건 대기: ① 연간일정 화면에 브런치Ⅲ 링크(u=1281) 배치 ② **전시 종료 항목에도 전시 상세 링크 연결**(현재 전시 시작에만 있음) — 클코캘린더 세션에서 처리.
