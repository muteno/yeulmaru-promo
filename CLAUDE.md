...
- **판매 추이 차트 = 정본 고정(운영자 260805 「이 차트를 디자인 정본으로 고정시켜서 참고하게해」)** = 기간이 있는 사업의 추이 차트는 **기틀 §2 #16 한 형태**뿐 — 앞 **메인 = 전시별 누계 꺾은선**(숫자·툴팁 전담) + 뒤 **보조 = 옅은 직각 스텝 누적 스택**(맥락 · 윤곽·점·툴팁 0). 조립 SSOT = `_bizDrawExhibMonthly`. ⚠ 자주 깨지던 축 셋 = **`shape:'hvh'`**(`hv`면 칸이 반 달 밀린다) · **`fill:'tozeroy'` 금지**(같은 바닥 겹치기 = 누적 아님) · **라벨 겹치면 생략 말고 180° 뒤집기**(세로 임계 ≥19). 게이트 = `tools/check_exchart.py`(하드 0 · 킬테스트 5/5) · 튜닝 시안 = `docs/reports/260805_전시_직각누적_플레이그라운드.html`.
...
- **사업비(전표실적·판매수수료·정산서매출) = 「그 공연에 딸린 사업 실적」(운영자 260812)** — 인덱스 = **사업NO**(`2025-공연-01`) + **회계구분**(거르기 축) + **연결키**(공연↔사업비 · 사람이 매단 관계가 자동 매칭을 이긴다). 씨앗 = `data/biz_finance.js`(**기계산출물** · `python3 tools/build_biz_finance.py` · 원본 = 최상단 「<연도>년 예술사업 대시보드.xlsx」) · **정본 = 시트 `운영_사업비`**(같은 NO가 시트에 있으면 시트가 이긴다 = 담당자 수정본을 재빌드가 못 되돌린다). ⚠ **파생값 저장 금지**(차액·수익율·계 = `_finCalc`가 센다 · 식 = `매출 ÷ (전표실적+판매수수료) × 100`) · ⚠ **사업NO는 위치가 아니라 정체성**(재빌드 때 이전 산출물의 NO 재사용 — 엑셀 중간에 줄이 껴도 안 밀린다). 권한 = 담당자 시트 **P열 `회계여부`**(맨 끝 append) → 판정 `_isAcct()` · **쓰기 허용은 Worker `checkAccountant` + `ACCT_WRITE_SHEETS` 한 장**으로 다시 정한다. 게이트 = `tools/check_finance.py`(하드 0 · 킬테스트 6/6) + `tools/smoke_finance.mjs`(런타임 · 킬테스트 3/3) · 전/후 = `docs/reports/260812_사업비_전후.html`.
