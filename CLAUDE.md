...
- **🔜 v2 방향(사용자 확정, 다음 작업)**: ① 맵에 각 홍보물 위치 팝업 **촤르륵(스태거) 등장** → **홍보물 선택** 가능하게 ② 우측 메뉴 = **세부메뉴 카테고라이징** ③ **지도 모달 ↔ 메뉴 양방향 동기화**(지도에서 메뉴 진입 · 메뉴에서 지도 확대) ④ **레이어 계층 구조는 사용자가 직접 설정해주기로 함** — 이 세션에서 계층 질문 전달함(답변 대기). 다음 세션은 사용자의 계층 정의부터 받고 시작할 것.
