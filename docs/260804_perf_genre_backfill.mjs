...
//
// [260804 실행 완료 · 라이브 실측] write ok(1,317건) → 재조회 검증 통과.
//   헤더 = 공연ID·대표공연명·연도·첫공연일·회차수·출처 **+ 장르1·장르혼합·장르출처**
//   장르출처 = 대장 1,202 · 운영자확정 22 · 대장최빈 2 · 미상 91
//   장르 분포 = 클래식 789 · 대중 151 · 기타 97 · 뮤지컬 80 · 발레/연극 73 · 어린이·가족 33 · 시즌 2 · 국악 1 · 공란 91
//   교차 확인 = 2023~2024 클래식 128건(예매 질의 「23~24년 클래식 2회 이상」의 공연 축이 이걸로 선다).
//   재실행은 안전하다 — 색인을 매번 대장에서 다시 만들어 덮으므로 멱등(누적 오염 없음).
...
// [운영자 260804 확정] 장르 결손 22건 전건 — 제목 목록을 보고 **운영자가 직접 정한 값**이다(세션 추정 아님 · 기틀 §3 묻고-편입).
//   대장 원장은 여전히 공란이다 — 이 표는 색인(장르 SSOT)에만 반영된다.
// ⚠ 어휘 확장 2종(`국악`·`시즌`) = 운영자 지정. 기존 6종(클래식·뮤지컬·발레/연극·대중·어린이·가족·기타) 밖이라
//   화면 장르 배색(--c1~--c6, 6색)에는 대응 색이 없다. 지금은 **색인 열에만 살고 화면이 읽는 축은 대장**이라
//   렌더 영향 0이지만, 장르별 차트를 색인 기준으로 옮기는 날엔 색 2개가 같이 필요하다(기틀 §3 묻고-편입 대상).
const OPERATOR_GENRE = {
  '141009_01': '클래식',   // 음악이흐르는광장영화관 오프닝 축하연주회 (2014)
  '201229_02': '뮤지컬',   // 창작뮤지컬 <별이쏟아진다> (2020)
  '170120_01': '국악',     // 제3회 여수청소년 국악관현악캠프 어울림 (2017) — 어휘 신설
  '220913_01': '시즌',     // 예울마루 개관 10주년 기념 프린지 콘서트 및 야외 영화제 (2022) — 어휘 신설
  '220917_01': '시즌',     // 예울마루 개관 10주년 기념 G콘서트 (2022) — 어휘 신설
