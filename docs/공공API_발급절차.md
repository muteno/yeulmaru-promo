...
## ⛔ 260802 현재 — 신청 자체가 막혀 있다 (포털 전환 작업)

공공데이터포털 공지 **「[안내] 공공데이터포털 개편에 따른 일부 서비스 이용 제한 안내」(2026-07-24 등록)** 실측:

- **작업 기간 = 2026-07-29(수) 19:00 ~ 2026-08-02(일) 18:00** (진행 상황에 따라 변동 가능)
- **이용 제한**: 회원 로그인·회원가입 / **오픈API 신규 활용신청** / **마이페이지(인증키 신규 발급·관리)** / 로그인 기반 서비스 일체
- **이용 가능**: 데이터 목록 검색·조회, 파일데이터 다운로드
- ✅ **기존 오픈API 호출은 정상** — 공지 명시분이며, 260802 16:10 KST에 두 게이트웨이를 직접 찔러 확인했다
  (`https://apis.data.go.kr/B553881/Parking/PrkRealtimeInfo` · `https://apis.data.go.kr/B551011/DataLabService/locgoRegnVisitrDDList`
  → 둘 다 **HTTP 401 Unauthorized** = 게이트웨이 정상 가동, 키만 무효인 상태)

**→ 아래 0~2단계(로그인·활용신청·인증키 확인)는 2026-08-02 18:00 KST 이후에 시작한다.**
③KOPIS(§3)와 ④여수시 문의(§4)는 이 작업과 무관하므로 지금 진행해도 된다.
...
| 단계 | 페이지 |
|---|---|
| 0-1. 로그인 | https://www.data.go.kr/uim/login/loginView.do |
...
| 포털 전환 작업으로 신청 차단 중 | 공지 원문 조회 + 게이트웨이 401 실측 | ✅ 확인 (~08-02 18:00 KST) |
