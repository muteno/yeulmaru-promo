...
> ⚠️ **지연폭은 고정이 아니다.** 260810 재확인 시에도 최신은 여전히 `20260711` — 그 사이 원본이 한 번도
> 갱신되지 않아 지연이 **23일 → 30일로 벌어졌다.** 그래서 앱은 지연폭을 상수로 박지 않고
> `/api/visitors` 응답의 **`lagDays`(매 갱신 시 실측)** 를 그대로 표시한다.
...
### 2-E. 구현 — Worker `GET /api/visitors` (260810 반영 완료)

- **소스**: `src/index.js` — `visitorIndex()` / `visitorRefresh()` / `visitorLatestYmd()` / `visitorBuild()`
- **인증**: 앱 로그인(`X-App-Password`) 필요. 키는 Worker 시크릿 **`DATAGO_KEY`**
  (미설정이면 공휴일용 `KASI_KEY`로 폴백 — 같은 포털·같은 계정 키라 그대로 동작한다).
  Encoding/Decoding 어느 판을 넣어도 되게 `%` 포함 여부로 분기한다(공휴일 코드와 같은 관용구).
- **동작**: ①`lagDays`가 고정이 아니므로 **데이터가 차 있는 마지막 날을 매번 탐색**한다
  (7일 간격 역방향 최대 9회 → 히트 지점에서 하루씩 6일 전진) ②그 날짜로 끝나는 **28일 창**을 한 번에 받아
  ③여수·순천·광양 × **외지인(b)** 만 골라 요일 평균과 지수를 계산.
- **캐시**: KV `visitors:idx` 24시간 + `visitors:idx:last`(stale-while-revalidate) — `/api/gcal`과 같은 축.
  원본이 하루 단위라 더 자주 받을 이유가 없고, 개발계정 1,000회/일 제한도 지킨다(1회 갱신 ≈ 12콜).
- **응답**:
  ```json
  { "ok": true, "from": "20260614", "to": "20260711", "lagDays": 30,
    "dow": ["월요일", "...", "일요일"],
    "cities": { "여수": { "avg": [56847, ...], "idx": [0.87, ...], "peak": 5 }, "순천": {}, "광양": {} } }
  ```
  `idx` = 그 요일 평균 ÷ 전체 요일 평균(1.00 기준) · `peak` = 가장 붐비는 요일의 인덱스.
- **미설정·실패 시** `ok:false` + `cities:{}` + `setup:true` → 프론트는 조용히 미표시(기존 화면 무영향).
- **실측 검증(260810)**: 실제 키로 호출해 `from 20260614 ~ to 20260711 · lagDays 30` 산출 확인.
  여수 지수 = 월 0.87 / 화 0.86 / 수 0.80 / 목 0.87 / 금 1.04 / **토 1.34** / 일 1.22, `peak=토요일`.
  KV 재조회 시 캐시 히트 확인.
