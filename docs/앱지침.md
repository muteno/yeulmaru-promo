...
- **동기화 생명선** = ①비활성→활성 복귀 시 `window.nmRefresh` 자동 호출(이 앱 정본 훅 = `_checkForUpdates` 방식B 뱃지 축 — 45s 폴링을 복귀 즉시로 앞당김) ③자기 문서 HEAD ETag 대조로 **새 배포 감지 → 한가할 때 자동 리로드**(sw.js는 `req.method !== 'GET'` 조기 통과라 HEAD가 SW를 우회 = 항상 진짜 네트워크 · 세션은 sessionStorage 생존 + 세션복원 IIFE라 리로드 안전). 원본 ②(Access 로그인 만료 자가치유)는 CF Access 전용 신호라 이 인프라(GitHub Pages+PIN/MSAL)에 대응물이 없어 **비배선**.
  - 한가 판정 `busy()` = 입력 포커스·contenteditable·열린 모달(`.modal-bg.show`/`.confirm-bg.show`/`dialog[open]`/z99999 고정 오버레이) — 작업 중 강제 이탈 0(감지는 다음 복귀로 이월). 페이지 자체 선언 훅 = `window.nmSyncBusy`.
  - 경계: 복귀 트리거뿐이라 탭을 한 번도 안 떠나는 화면은 다음 복귀·수동 새로고침까지 구코드 유지(노뮤트 원본과 동일 계약). signage는 입력칸 0 + 자체 시간·manifest 폴링 갱신이라 배선 대상 아님.
