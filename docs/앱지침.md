...
- **담당자 시트 = 16열(A~P) 확정(positional SSOT)**: `A`담당부서 `B`담당자 `C`직위 `D`휴직여부 `E`홍보여부 `F`기관여부 `G`공연여부 `H`전시여부 `I`예술교육여부 `J`대관여부 `K`PIN `L`비밀번호 `M`계정여부 `N`관리자여부 `O`이메일 `P`전화번호(260707 append — 티켓 등록 신청 담당자 자동 채움용, **시트 P1 헤더 셀에 `전화번호` 기입해야 읽힘**). `SHEET_CONFIG.manager.columns` 배열이 이 순서와 **1:1**이어야 하며, `saveSheetRow`(등록 폼)·`saveInlineRow`(인라인 편집)·`buildSheetListHtml`(목록 표) 3곳이 이 배열 하나에 의존한다.
