...
├─ enneagram_book.html           ← 소스: 교재 (1~3부 = 강의 재구성·관계 중심 · 4부 = 웹 수집 심화)
...
- **교재 4부(12~17장)는 강의 밖 지식**: 체계의 계보·전통 4대 지도(격정/미덕/고착/신성한 생각)·발달 수준 9단계·하위유형 27종(역행유형★)·오판별·검증과 비판. 출처가 1~3부와 다르므로 **표지와 각 장 말미에 출처를 표시**한다(원천 = `docs/reports/20260726_211242_애니어그램_웹리서치_보강_v1.md`). 4부를 손대면 `build/build_merge.py`로 **배포물 재빌드 필수** — 앱 이론 탭은 `enneagram_all.html` 안의 사본을 본다.
