 ...
 *   ③ 「도구가 통과했다」 ≠ 「검색이 됐다」 — 정의는 받아들여졌는데 실행만 실패하면 HTTP는 200이고
 *      결과 블록만 `{error_code}`로 온다(모델은 그걸 「검색이 막혔다」로 읽고 답한다 = 조용한 실패).
 *      그래서 실패 사유를 세어 `searchErrs`로 올린다 — 260812 이 계량이 없었으면 「검색 2회」만 보이고
 *      **왜 못 찾았는지**(코드 실행 too_many_requests)를 못 짚었다.
 *
