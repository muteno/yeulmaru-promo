...
const SUBDIRS = ['hwp', 'office', 'slim', 'promo', 'press'];   // promo = AI 홍보 전략 입력 팩(pa*.in.json — 런 실패로 남은 잔재 회수용, 정상 런은 워크플로가 즉시 삭제)
//   press = 보도자료 만들기(pr*.out.docx · 자료 원본 pr*.src<N>.<ext>는 정상 런이 즉시 삭제 — 여기는 실패 잔재용)
