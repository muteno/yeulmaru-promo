...
NAME_MAP = {
    # 값 = (사업명, 비고). 비고는 운영자가 `/` 오른쪽으로 직접 준 값이다 — 원본 괄호에서 뽑지 않는다.
    2024: {
        "2024-공연-01": ("2024 신년음악회", "클래식 1"),
        "2024-공연-02": ("2024 브런치 콘서트", "클래식 2"),
        "2024-공연-03": ("2024 GS칼텍스 예울마루 위크", "문화나눔"),
        # ⚠ 2025-공연-04와 같은 자리에서 운영자 원문의 여는 괄호가 안 닫혀 있었다 — 닫아서 적는다.
        "2024-공연-04": ("2024 공모 사업 (뮤지컬 <장영실과 함께하는 별별GPT> · 무풍)", "공모"),
        "2024-공연-05": ("2024 헬로!오페라", "클래식 4"),
        "2024-공연-06": ("조재혁 피아노 리사이틀", "클래식 3"),
        # 운영자 확인: 「- 양성원 이라고 써진 걸 말하는 거임」 = 씨앗의 `클래식시리즈2 (양성원)` 행.
        "2024-공연-07": ("2024 실내악 페스티벌 <명작의 울림>", "클래식 5"),
        # 2024-공연-08 `클래식공연 3 (3테너)` = 운영자 목록에 없다 — 안 건드린다(씨앗 이름 유지).
        "2024-공연-09": ("발레 <호두까기 인형>", "크리스마스 1"),
        "2024-공연-10": ("뮤지컬 <썸데이>", "협력 1"),
        "2024-공연-11": ("뮤지컬 <폴리팝>", "아동극 1"),
        "2024-공연-12": ("뮤지컬 <친구의 전설>", "아동극 2"),
        "2024-공연-13": ("뮤지컬 <노트르담 드 파리>", "뮤지컬 1"),
        "2024-공연-14": ("뮤지컬 <젠틀맨스 가이드: 사랑과 살인편>", "뮤지컬 2"),
        "2024-공연-15": ("뮤지컬 <오즈의 의류수거함>", "청소년 1"),
        "2024-공연-16": ("연극 <시간을 파는 상점>", "연극 1"),
        "2024-공연-17": ("윈터워크-재즈 피아노, 트리오, 재즈 빅밴드와 함께하는 특별한 음악 산책", "크리스마스 2"),
        "2024-공연-18": ("아레나 디 베로나 오페라 페스티벌 <라보엠> 공연 실황", "크리스마스 3"),
    },
    2025: {
        "2025-공연-01": ("2025 신년음악회", "클래식 1"),
        "2025-공연-02": ("백건우와 모차르트", "클래식 2"),
        "2025-공연-03": ("스페인 국립 플라멩코 발레단 내한공연 <아파나도르>", "협력 2"),
        # ⚠ 운영자 원문의 여는 괄호가 안 닫혀 있었다 — 닫아서 적는다(안 닫으면 화면·시트에 그대로 새어 나간다).
        "2025-공연-04": ("2025 공모사업 (창작 발레 갓 · 연극 <꽃의 비밀> · 열혈건반 <쇼팽, 그리고 쇼팽>)", "공모"),
        "2025-공연-05": ("2025 GS칼텍스 예울마루 위크", "문화나눔 1"),
        "2025-공연-06": ("2025 헬로!오페라", "클래식 4"),
        "2025-공연-07": ("2025 실내악 페스티벌 <동유럽의 춤>", "클래식 5"),
        "2025-공연-08": ("연극 <내일은 내일에게>", "연극 1"),
        "2025-공연-09": ("발레 <호두까기 인형>", "크리스마스 1"),
        "2025-공연-10": ("2025 브런치 콘서트", "클래식 3"),
        "2025-공연-11": ("가족 뮤지컬 <넌 특별하단다>", "아동극 1"),
        "2025-공연-12": ("뮤지컬 <명성황후>", "뮤지컬 1"),
        "2025-공연-13": ("가족 뮤지컬 <넘버블록스>", "아동극 2"),
        "2025-공연-14": ("뮤지컬 <설민석의 한국사 대모험 : 안중근>", "아동극 3"),
        "2025-공연-15": ("뮤지컬 <시카고>", "협력 1"),
        "2025-공연-16": ("뮤지컬 <킹키부츠>", "뮤지컬 2"),
        "2025-공연-17": ("뮤지컬 <김종욱 찾기>", "청소년 1"),
    },
    # 2026 = 260813 운영자 재확정(18줄). 앞판의 8건을 **이 목록이 덮는다**(비고도 새 값으로).
    2026: {
        "2026-공연-01": ("2026 신년음악회", "클래식 1"),
        "2026-공연-02": ("2026 실내악 페스티벌 <실내악, 그 이상의 세계>", "클래식 2"),
        # ⚠ 여기도 운영자 원문의 여는 괄호가 안 닫혀 있었다 — 닫아서 적는다(2024·2025와 같은 처리).
        "2026-공연-03": ("2026 공모 사업 (김영욱×콜레기움 무지쿰 서울 <8 Seasons> · 뮤지컬 <이상한 나라의 춘자씨> · 연극 <그때도 오늘>)", "공모 1"),
        "2026-공연-04": ("한국페스티벌앙상블 <세상에서 가장 편한 음악>", "클래식 3"),
        "2026-공연-05": ("국립심포니오케스트라 <피터와 늑대 & 어미 거위>", "협력 1"),
        "2026-공연-06": ("연극 <노인의 꿈>", "연극 1"),
        "2026-공연-07": ("2026 헬로!오페라", "공모 2"),
        "2026-공연-08": ("조재혁 피아노 리사이틀", "클래식 4"),
        "2026-공연-09": ("국립현대무용단 <트리플 빌>", "무용 1"),
        # ⚠ 「피아노 피아노」는 씨앗에 두 줄(공연-10 · 공연-11)이 걸린다 — 운영자 목록도 한 줄뿐이라 11만 바꾼다.
        #   공연-10(「피아노&피아노」)은 안 건드린다(중복인지 별개 공연인지는 원본을 아는 사람이 정할 일).
        "2026-공연-11": ("다비드 바뱅 & 아드리앙 몽도 <피아노 피아노>", "협력 2"),
        # 씨앗 `연극1(소극장, 미정)` = 2026에 남은 하나뿐인 연극 자리. 운영자 목록의 「쉬어 매드니스」가 여기 선다.
        # ⚠ 비고 「연극 1」이 공연-06과 겹친다 — 운영자가 준 값 그대로 적었다(고칠지는 운영자가 정한다).
        "2026-공연-12": ("연극 <쉬어 매드니스>", "연극 1"),
        # 운영자 확인: 「- 재단 20주년 기념 음악회를 말함」 = 씨앗의 `문화나눔콘서트(재단20주년 음악회)` 행.
        "2026-공연-13": ("2026 여수섬박람회 기념 음악회", "문화나눔 1"),
        "2026-공연-14": ("뮤지컬 <미세스 다웃파이어>", "뮤지컬 1"),
        "2026-공연-15": ("2026 브런치 콘서트", "클래식 6"),
        "2026-공연-16": ("뮤지컬 <100층 짜리 집>", "아동극 1"),
        "2026-공연-17": ("뮤지컬 <그날들>", "뮤지컬 2"),
        # 2026-공연-18 `아동극2(미정)` = 운영자 목록에 없다(아직 미정) — 안 건드린다.
        "2026-공연-19": ("뮤지컬 <러커스 더 스쿨>", "청소년 1"),
        "2026-공연-20": ("발레 <호두까기 인형>", "크리스마스 1"),
    },
}

# ⚠ 대응표에 있는 NO가 씨앗에 없는 것은 **하드 실패가 아니라 알림(ℹ)** 이다.
#   사업이 원본 엑셀에서 정당하게 빠질 수 있고, 무엇보다 `check_finance` ⑧(NO 안정성)이
#   「한 줄을 지우고 재빌드」로 정체성을 시험하는데 그때마다 빌드가 죽어 게이트가 오탐을 냈다(260813 실측).
#   진짜 위험(정산서 값이 안 반영되는 것)은 `check_finance` ⑭가 실제 xlsx와 대조해 하드로 잡는다.
def apply_names(rows, year, warn):
    """사업명·비고를 운영자 확정 값으로 바꾼다. 반환 = 바뀐 건수.
    ⚠ 비고(`tag`)는 운영자가 준 값을 그대로 쓴다 — 원본 괄호에서 뽑지 않는다(첫 판은 뽑았고, 괄호가 없던 행에서
      사업명이 통째로 비고에 복사됐다). 빈 문자열이면 비고를 안 남긴다 = 화면이 회계구분으로 물러난다."""
    table = NAME_MAP.get(year) or {}
    if not table:
        return 0
    by_no = {r.get("no"): r for r in rows if r.get("no")}
    n = 0
    for no, pair in sorted(table.items()):
        nm, tg = pair if isinstance(pair, tuple) else (pair, "")
        r = by_no.get(no)
        if r is None:
            warn.append("ℹ 사업명 대응표의 `%s`가 %d년 씨앗에 없다 — 그 사업이 원본에서 빠졌다면 표에서도 지워라" % (no, year))
            continue
        if tg:
            r["tag"] = tg
        if r["name"] == nm:
            continue
        print("[사업비] %d년 사업명 %s %-24s → %-40s (비고 %s)" % (year, no, str(r["name"])[:24], nm, tg or "-"))
        r["name"] = nm
        n += 1
...
def settle_apply_rev(rows, hit, year, warn):
    ...
    for no, amt in sorted(hit.items()):
        r = by_no.get(no)
        if r is None:
            warn.append("ℹ 정산서 매출 대응표의 `%s`가 %d년 씨앗에 없다 — 그 사업이 원본에서 빠졌다면 표에서도 지워라" % (no, year))
            continue
