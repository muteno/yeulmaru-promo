...
<title>고객 분류 조건 문장 + 끝자리·장르 분리 — 전/후 (260814)</title>
<style>
...
<h1>고객 분류 — 조회 실패 규명 · 조건 문장(변수 5개) · 휴대폰 끝자리 · 장르 분리</h1>
...
${shot('후(1차) — 그 자리에서 말하고 멈춘다', false, '260814_고객분류_못읽음_후', '후: 없는 축이면 그 자리에서 안내하고 조회하지 않는다', '같은 질문 · 실측 = 예울이 <b>왕복 0회</b>(즉시) · <code>_segLast</code> 그대로 = <b>조회 안 함</b>. <b>휴대폰 끝자리는 그 뒤 ④에서 축으로 붙어</b> 이제 실제로 찾습니다 — 이 화면은 아직 없는 축(이름·공연명·연령·금액·매수)에 그대로 적용됩니다.')}
</div>
<ul style="font-size:12.5px">
<li><b>없는 축 사전 차단</b> — 이름·공연명·연령·금액·매수(그리고 숫자 없이 「번호로 찾아줘」)를 짚은 질문은 <b>보내기 전에</b> 그 자리에서 안내(30초~2분 기다린 끝에 엉뚱한 조건이 오는 걸 막는다). 읽힌 축이 하나라도 있으면 그것만은 문장에 채워 둡니다.</li>
...
<h2>④ 휴대폰 끝자리로 찾기 (260814-2 운영자 「검색되게 해야되」)</h2>
${shot('후 — 같은 질문이 이제 그 사람을 찾는다', false, '260814-2_끝자리_후', '끝자리 조회 결과', '같은 문장 <code>0789 끝자리인 번호로 예매한 이력을 확인해줘</code> · <b>예울이 왕복 0회</b>(로컬 즉시) · 문장 맨 앞에 「번호 뒤 0789」 변수가 선다.')}
<ul style="font-size:12.5px">
<li>축 = <code>tail</code>(휴대폰 뒤 2~4자리). 예매집계의 <b>회원키가 곧 번호</b>라 이름 조회 전에 걸러집니다 — 회원 시트와의 조인은 종전 그대로.</li>
<li><b>밖으로 안 나갑니다</b> — 끝자리가 읽히면 그 자리에서 해석이 끝나 예울이(질문이 공개 저장소에 남는 경로)를 <b>아예 안 부릅니다</b>. 온전한 번호를 적어도 <b>뒤 4자리만</b> 조건에 담습니다.</li>
<li>채팅(예울이)에서 물으면 <b>관리자에게만</b> 답합니다(명단 화면과 같은 게이트).</li>
<li>문장에는 <b>걸었을 때만</b> 섭니다 — 평소엔 안 쓰는 축이라 「1줄로 말끔히」를 지킵니다(해제하면 사라짐).</li>
<li>버그 하나 같이 잡음: 온전한 번호 <code>010-8800-1299</code>의 숫자를 기간 파서가 <b>「2010~2088년」</b>으로 읽어 없는 기간을 얹고 있었습니다 → 번호 조각을 <b>맨 먼저</b> 떼어 내도록 순서 고정.</li>
</ul>

<h2>⑤ 장르 쪼개기 (운영자 「발레무용 하나고 연극이 별개 · 대중 국악 기타 다 별개」)</h2>
${shot('후 — 발레·무용 / 연극이 각각 선다', false, '260814-2_장르목록_후', '장르 목록 — 발레·무용과 연극 분리', '옆 숫자 = 그 기간에 그 장르를 본 <b>사람 수</b>(실측 · 목데이터 12명 기준).')}
<ul style="font-size:12.5px">
<li>구판은 <b>앱이</b> 발레·무용·연극을 <code>발레/연극</code> 한 이름으로 <b>밀어 넣고</b> 있었습니다(<code>_BK_ALIAS</code>) — 시트가 나눠 적어도 화면에서 다시 합쳐졌습니다. 이제 화면 장르 = <b>시트 값 묶음</b>(<code>_BK_GMAP</code>)이라 「연극」은 연극으로, 「발레/무용」은 발레·무용으로 갑니다.</li>
<li>목록은 <b>시트에 실제로 있는 값</b>까지 합쳐 만듭니다 — 어휘를 손으로만 적으면 시트에 새 값이 생겼을 때 그 사람들이 조용히 사라집니다.</li>
<li>⚠ <b>시트에 이미 「발레/연극」으로 적힌 회차는 못 나눕니다</b>(그 안에 둘이 섞여 있음). 지어내 나누지 않고 <b>그 값 그대로 한 줄 더</b> 띄우고(인원 병기), 발레·무용/연극을 고르면 결과 아래에 <b>「이만큼은 안 나뉘어 빠졌다」</b>를 적습니다. 실제로 나누려면 원천 <code>장르1</code>을 나눠 적고 <code>node docs/260804_booking_agg.mjs</code>를 다시 돌려야 합니다(앱이 못 하는 몫).</li>
<li><b>기타 광역시</b> = 운영자 확정(전남·광주 밖 광역시급 — 부산·대구·인천·대전·울산·세종) · <b>대중·국악·기타</b>도 확정대로 별개 장르 유지.</li>
</ul>

<h2>⑥ 실측</h2>
<table>
<tr><th>검사</th><th>도구</th><th class="n">결과</th></tr>
<tr><td>조회 실패 경로 · 끝자리 축 · 장르 분리 · 이상/이하 · 거주지 묶음 · 조건 문장</td><td><code>tools/scratch/probe_seg_nlfail.mjs</code></td><td class="n"><b class="ok">38/38</b></td></tr>
...
const out = join(ROOT, 'docs/reports/260814-2_고객분류_끝자리_장르분리_전후.html');
