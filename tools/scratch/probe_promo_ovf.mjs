// 좁은 화면 가로넘침 — 이 변경 전/후 공통으로 도는 최소 측정(선택자 무관).
...
for (const [W,H] of [[390,844],[360,740]]) {
  ...
  const pre = await page.evaluate('document.documentElement.scrollWidth-document.documentElement.clientWidth');
  ...
  const post = await page.evaluate(()=>{
    ...
    return {ovf:document.documentElement.scrollWidth-document.documentElement.clientWidth,
            modalW:+m.width.toFixed(1), fromRight:+(m.right-x.right).toFixed(1), fromTop:+(x.top-m.top).toFixed(1)};
  });
  console.log(`${W}x${H}  모달 열기 전 가로넘침 ${pre} → 연 뒤 ${post.ovf}  (모달폭 ${post.modalW} · X 우변 ${post.fromRight}/상변 ${post.fromTop})`);
  await page.close();
}
await browser.close();
